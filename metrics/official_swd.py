...
def _sliced_wasserstein(a, b, random_sampling_count, random_projection_dim):
  ...
  for _ in range(random_sampling_count):
    ...
    means.append(wdist)
    # print(math_ops.reduce_mean(means))
...
def sliced_wasserstein_distance(real_images,
  ...
  Raises:
      ...
      same size.
  """
  # he = real_images.tensor_shape()[1]
  # print(he)
  height = real_images.shape[1]
  # height = tf.shape(real_images)[1]
  print(height)
  ...
  for lod in range(len(resolutions)):
    if not use_svd:
      # _sliced_wasserstein(patches_real[lod], patches_test[lod],random_sampling_count, random_projection_dim)))
      scores.append(
...
if __name__ == '__main__':
    with tf.Session() as sess:
        # real_img_path = "F:/X---Y/extend-one/"
        # fake_img_path = "F:/X---Y/extendY-1-1/"
        # [(0.014279558, 0.014124566), (0.011000062, 0.011390291)]
        # [(0.016658349, 0.051532604), (0.014511413, 0.29051968)]

        # [(nan, nan), (nan, nan), (0.022334816, 0.035637181), (0.020762699, 0.10417665)]
        real_img_path = "F:/swd/new3/real/"
        fake_img_path = "F:/swd/new3/fake1/"
        filenames1 = glob.glob(os.path.join(real_img_path, '*.*'))
        filenames2 = glob.glob(os.path.join(fake_img_path, '*.*'))
        real_images = []
        fake_images = []
        for filename in filenames1:
            kk = np.expand_dims(get_images(filename),0)
            real_images.extend(kk)
        real_images= np.array(real_images)
        real_tensor = tf.convert_to_tensor(real_images)

        for filename in filenames2:
            kk = np.expand_dims(get_images(filename),0)
            fake_images.extend(kk)
        fake_images= np.array(fake_images)
        fake_tensor = tf.convert_to_tensor(fake_images)
        # print(real_tensor)
        # print(real_images)
        # print(fake_tensor)
        # print(fake_images)
        ...
        kk = sliced_wasserstein_distance(real_tensor,
                                         fake_tensor,
                                    resolution_min=16,
                                    patches_per_image=64,
                                    patch_size=7,
        ...
        print(kk[0])
        print(sess.run(kk))
        print(sess.run(kk[0],feed_dict={real_tensor:real_images,fake_tensor:fake_images}))
